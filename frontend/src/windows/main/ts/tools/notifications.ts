...
interface NotificationEventMap {
  ...
  export class Notification {
	...
	private process: SpawnEventEmitter & Promise<number> | null = null;
  ...
  /**
   * Removes the notification if it has a group ID.
   * @throws If the notification doesn't have a group ID.
   */
  public async remove(): Promise<void> {
    if (!this.options.group) {
      throw new Error('Cannot remove notification without a group ID');
    }

    const alerter = libraryPath('notifications');
    await spawn(alerter, ['-remove', this.options.group]);
  }
