...
export class RobloxInstance {
    ...
    private async setupLogsWatcher() {
        ...
        while (this.latestLogPath == null) {
            if (tries < 1) {
                throw new Error(`Couldn't find a .log file created less than 3 seconds ago in "${this.logsDirectory}". Stopping.`);
            }
            const latestFile = (
    ...
    private processLines(lines: string[]) {
        ...
        const messageLines = lines.filter(line => Patterns.find(p => 
            p.event === 'GameMessageEntry')?.regex.test(line));
        
        for (const line of messageLines) {
            const match = line.match(Patterns.find(p => 
                p.event === 'GameMessageEntry')!.regex);
            if (match) {
                this.emit('gameEvent', { 
                    event: 'GameMessageEntry', 
                    data: match[0] 
