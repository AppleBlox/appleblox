<script lang="ts" type="module">
	...
	import FfButtonsCustom from '../../pages/Settings/FFButtonsCustom.svelte';
	import ModsUi from '../../pages/Settings/ModsUI.svelte';
		...
		for (const inter of section.interactables || []) {
			switch (inter.options.type) {
				...
				case 'dropdown':
					...
					break;
				case 'number':
					sections[section.id][inter.id] = [inter.options.value] || [
						inter.options.default,
					];
					break;
				case 'file':
			...
			if (loadedSettings) {
				...
				for (const category of Object.keys(loadedSettings)) {
					...
					for (const [key, value] of Object.entries(loadedSettings[category])) {
						if (
							key in sections[category] &&
							typeof value === typeof sections[category][key]
						) {
							upToDateSettings[category][key] = value;
						} else {
							console.warn(
								`Settings value not take into account: ${panel.id}.${category}.${key} = ${value}`
							);
						}
					}
				}
			...
			<div class="mt-5">
				...
				{#each section.interactables || [] as inter}
					{#if inter.options.type !== 'button' && inter.options.type !== 'ff_buttons_custom'}
						<Separator
							class="my-3 bg-gray-300 opacity-25"
							el={undefined}
							decorative={true}
						/>
					{/if}
					<div
						class={`flex items-center ${inter.toggle ? (sections[section.id][inter.toggle] ? '' : 'grayscale cursor-not-allowed opacity-60 select-one pointer-events-none') : ''}`}
					>
									...
									<Tooltip.Trigger>
										...
										>
											{#if inter.options.icon?.component}
												<svelte:component
													this={inter.options.icon.component}
													class={cn(
														inter.options.icon.props,
														'h-5 w-5 mr-2'
													)}
												/>
											{:else if inter.options.icon?.src}
												<img
													src={inter.options.icon?.src}
													alt="Panel Icon"
													class={cn(
														inter.options.icon.props,
														'h-5 w-5 mr-2'
						...
						{:else if inter.options.type === 'boolean'}
							<Switch
								...
								on:click={() => {
									dispatch('switchClicked', {
										id: inter.id,
										state: !sections[section.id][inter.id],
									});
								}}
							/>
						...
						{:else if inter.options.type === 'file'}
									...
									try {
										...
										if (sections[section.id][inter.id]) {
											dispatch('fileRemoved', {
												id: inter.id,
												filePath: sections[section.id][inter.id],
											});
										...
										let options = {
											...
											filters: [
												{
													name: 'Font files',
													extensions: inter.options.accept,
												},
											],
										};
										...
										const entries = await os.showOpenDialog(
							...
							>
								{#if sections[section.id][inter.id]}
									...
									<p
										class="inline-block align-middle overflow-hidden whitespace-nowrap overflow-ellipsis [direction:rtl] w-full"
									>
										{path.basename(sections[section.id][inter.id])}
									</p>
								{:else}
						...
						{:else if inter.options.type === 'dropdown'}
							...
							>
								<Select.Trigger
									class="w-[180px] dark:bg-neutral-900 bg-neutral-300 ml-auto mr-4 border-none"
								>
									<Select.Value
										class="text-black dark:text-white"
										placeholder={inter.options.default.label}
									/>
								</Select.Trigger>
								<Select.Content
								...
								>
									<Select.Group>
										{#each inter.options.list || [] as item}
											<Select.Item value={item} label={item.label}
